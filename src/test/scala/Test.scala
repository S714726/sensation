...
trait WorkingQuery extends Query {
  override def fetchQuery(p: QueryParam)(implicit apiKey: EchoNestKey): Elem = {
    XML.loadString(p match {
...
class EchoNestSpec extends FunSpec {
  ...
  describe("Artist") {
    artist = new Artist(Some(InputName("Placeholder")), None) with WorkingQuery
    it("should parse & store queries") {
      assert(artist.songs.head.data(Name) == "I Love You")
    }
  }

  describe("Song") {
    song = new Song(None, InputId("Placeholder")) with WorkingQuery
    it("should parse & store queries") {
